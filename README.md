...
| Telegram 收取 | ✅ | 长轮询(无需公网地址)→ update_id 幂等 → 发送者授权 → 频率限制 → 一次性按钮 |
...
- 后端所有 `/api/*`(除少数白名单如 `/health` / `/version` / `/version/check-update` / `/api/proxy/status`)都要求 `X-API-Key` 请求头。`/api/proxy/status` 进白名单,是因为首次运行被 AuthGate / OvhCredsGate 全屏遮罩拦着,右上角的代理指示器登录前就得可见(代理地址已脱敏)
...
| 环节 | 作用 | 失败响应 |
|---|---|---|
| **发送者授权** | 只认 `tgChatId` 配置的那个 chat;群聊还要求 user id 在 `TG_ALLOWED_USER_IDS` 白名单里。这道检查排在幂等写入**之前** —— 否则谁都能先占号,把 Telegram 真发的同号 update 挤成"重复投递" | `403 unauthorized_actor` |
